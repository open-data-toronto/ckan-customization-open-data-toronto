...
def update_package(context):
    ...
    for r in resources:
        ...
        # These are sorted through below
        if r.last_modified is None:
            last_refreshed.append(r.created)
        else:
            last_refreshed.append(r.last_modified)
    ...
    if (
        ...
        old_last_refreshed = tk.get_action("package_show")(context, {
            "id": package.id})["last_refreshed"]
    else:
